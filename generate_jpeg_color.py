...
def generate_jpeg(filename, ascii_file, comment="ASCII JPEG"):
    with open(filename, "wb") as f:
        ...
        write_marker(f, 0xFFC4, struct.pack("B", 0x01) + dc_lengths + dc_symbols)  # DC table for CbCr
        ...
        write_marker(f, 0xFFC4, struct.pack("B", 0x11) + ac_lengths + ac_symbols)  # AC table for CbCr

        # SOF (Start of Frame) for Baseline DCT, non-differential, Huffman coding
        sof_data = struct.pack(">BHHB" + "BBB" * 3,
                               ...
                               2, 0x11, 0,  # Cb component: ID=2, subsampling=1x1, quant table=0
                               3, 0x11, 0)  # Cr component: ID=3, subsampling=1x1, quant table=0
        write_marker(f, 0xFFC0, sof_data)

        # SOS (Start of Scan)
        sos_data = struct.pack("B", 3) + \
                   struct.pack("BB", 1, 0x00) + \
                   struct.pack("BB", 2, 0x11) + \
                   struct.pack("BB", 3, 0x11) + \
